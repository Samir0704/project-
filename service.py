...
def delete_todo():
    delete_todo_query = '''delete from todos where todo_id = %s;
    '''
    cur.execute(delete_todo_query)
    return utils.ResponseData('Todo Successfully deleted', True)


def edit_todo(todo_id: int, title: str, todo_type: TodoType):
    edit_todo_query = '''select * from todos where todo_id = %s;
    '''
    cur.execute(edit_todo_query, (todo_id,))

def update_todo( title: str, todo_type: TodoType):
    update_todo_query = '''update todos set title = %s,todo_type = %s where todo_id = %s;
    '''
    cur.execute(update_todo_query, (title, todo_type, session.session.id))
